    ...
    <View className='bg-primary flex-1'>
      <ScrollView contentContainerStyle={{
        ...
        <View className='flex-col items-start justify-center mt-5 px-5'>
          ...
          <MovieInfo label='Generes'  value={movie?.genres.map((g)=>g.name).join(' - ') || 'N/A'}></MovieInfo>

          <View className='flex flex-row justify-between w-1/2'>
            <MovieInfo label='Budget' value={`$${(movie?.budget || 0) / 1_000_000} million`}></MovieInfo>
            <MovieInfo label='Revenue' value={`$${Math.round(movie?.revenue || 0) / 1_000_000}`}></MovieInfo>
          </View>
